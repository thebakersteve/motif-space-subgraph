import {
  ...
  BidRemoved,
  BidShareUpdated,
} from '../types/SpaceExchange/SpaceExchange'
import { BigInt, log, store } from '@graphprotocol/graph-ts'
import { Ask, Bid, Space, Transfer } from '../types/schema'
import {
  createAsk,
  createBid,
  createInactiveAsk,
...
/**
 * Handler called when a `BidShareUpdated` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
export function handleBidShareUpdated(event: BidShareUpdated): void {
  ...
  space.creatorBidShare = bidShares.creator.value
  space.ownerBidShare = bidShares.owner.value
  space.prevOwnerBidShare = bidShares.prevOwner.value
  space.save()
...
/**
 * Handler called when the `AskCreated` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
export function handleAskCreated(event: AskCreated): void {
  ...
  let askId = space.id.concat('-').concat(space.owner)
...
/**
 * Handler called when the `AskRemoved` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
export function handleAskRemoved(event: AskRemoved): void {
  ...
  if (onChainAsk.amount.equals(zero)) {
    ...
    askId = tokenId.concat('-').concat(space.owner)
    let ask = Ask.load(askId)
    if (ask == null) {
...
/**
 * Handler called `BidCreated` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
export function handleBidCreated(event: BidCreated): void {
  ...
  let bidId = space.id.concat('-').concat(bid.bidder.toHexString())
...
/**
 * Handler called when the `BidRemoved` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
...
/**
 * Handler called when the `BidFinalized` Event is emitted on the Motif SpaceExchange Contract
 * @param event
 */
export function handleBidFinalized(event: BidFinalized): void {
  ...
  // https://github.com/ourzora/core/blob/master/contracts/SpaceExchange.sol#L349
  // Find the transfer by id and set the from address as the prevOwner of the space
